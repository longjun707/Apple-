...
export default function TwoFAModal({ open, accountId, phoneNumbers, onClose, onSuccess }: TwoFAModalProps) {
  ...
  const getPhoneLabel = (phone: PhoneNumber) =>
    phone.fullNumberWithCountryPrefix || phone.numberWithDialCode || `号码 ${phone.id}`
  ...
  const smsMutation = useMutation({
    ...
    onError: (mutationError) => setError(getErrorMessage(mutationError, '发送短信失败')),
  })

  const isBusy = verifyMutation.isPending || smsMutation.isPending
  ...
  return (
    <Modal open={open} onClose={handleClose} disableClose={isBusy}>
      <div className="p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button
              type="submit"
              disabled={code.length !== 6 || smsMutation.isPending}
            ...
            {phoneNumbers && phoneNumbers.length > 0 ? (
              phoneNumbers.map((phone) => (
                <button
                  ...
                  disabled={smsMutation.isPending || !accountId}
                ...
                >
                  ...
                  {smsMutation.isPending
                    ? '发送中...'
                    : smsSent && selectedPhoneId === phone.id
                      ? `重新发送到 ${getPhoneLabel(phone)}`
                      : `发送短信到 ${getPhoneLabel(phone)}`}
                </button>
              ))
            ) : (
